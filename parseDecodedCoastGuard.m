function [ msg13_char, msg13_num, msg5_char, msg5_num ] = parseDecodedCoastGuard(aisFile)
...
% message id 1-3
m22idx = find(nFields == 22);
 
% parse message id 1-3
m22Parts = regexp(rawData(m22idx),',','split');
m22Parts = m22Parts(2:end, :);
n16 = length(m22Parts);
...
%     dnum = datenum(sprintf('%s %s', msg13{1}, msg13{2}),date_fmt);
    dnum = datenum(msg16{8},date_fmt);
    MMSI = str2double(msg16{2});
%     navStatus = str2double(msg13{2});
    navStatus = 0;  % force to be 'under way' status since field is text
    SOG = str2double(msg16{9}); % 
    lon = str2double(msg16{7}); % 
    lat = str2double(msg16{6}); % 
    COG = str2double(msg16{10}); %
    trueHeading = str2double(msg16{11}); 
    
    IMO = str2double(msg16{4}); % prune off "IMO" at beginning 
    
    draught = str2double(msg16{15});    
    toBow = str2double(msg16{15});
    toStern = str2double(msg16{17});
    toPort = str2double(msg16{18});
    toStarboard = str2double(msg16{19});
